      ...
      <div className="container page-hero" id="career" data-reveal>
        ...
        <article className="page-card career-openings">
          ...
          {jobsQuery.isLoading ? (
            ...
            <ErrorMessage
              compact
              title="Unable to load current job openings."
              detail="The careers page copy is available, but the job listing feed is not responding right now."
            />
          ) : jobs.length ? (
                ...
                <article key={job.id} className="career-job">
                  ...
                  <div className="career-job__lists">
                    <div>
                      ...
                      <ul>
                        {(job.responsibilities.length ? job.responsibilities : [job.description]).slice(0, 3).map((item) => (
                          <li key={item}>{item}</li>
                        ))}
                      </ul>
                    </div>
                  ...
                  <div className="career-job__footer">
                    <p>{job.compensation_label || "Compensation shared during the hiring process."}</p>
                    <a className="landing-cta landing-cta--primary" href={content.status.primary.href}>
                      Apply for {job.title}
                    </a>
                  </div>
                </article>
            ...
            <div className="career-empty">
              There are no published openings right now. Use the contact action above and we will still help direct your inquiry.
            </div>
          )}
        </article>
      </div>
